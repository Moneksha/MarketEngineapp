...
ingest_feather.py
-----------------
Ingests RELIANCE 1-minute OHLCV data from a Feather file into PostgreSQL.

Usage:
    python scripts/ingest_feather.py

The script will:
1. Create the database Equity_db if it does not exist.
2. Create the table reliance_ohlcv if it does not exist.
...
# ── Config ────────────────────────────────────────────────────────────────────
FEATHER_PATH = "/Users/dmoneksh/Desktop/2026 data and codes/RELIANCE_1minute_2000-01-01_to_2026-01-01.feather"
...
def ensure_table_exists(engine):
    ...
    CREATE INDEX IF NOT EXISTS idx_reliance_ohlcv_date ON {TABLE_NAME} (date);
    """
    ...
    for i in range(0, len(df), CHUNK_SIZE):
        ...
        with engine.connect() as conn:
            conn.execute(text(f"""
                ...
                FROM __reliance_temp
                ON CONFLICT (date) DO NOTHING
            """))
            conn.execute(text("DROP TABLE IF EXISTS __reliance_temp"))
            conn.commit()
...
if __name__ == "__main__":
    print("=" * 60)
    print("  RELIANCE OHLCV Feather → PostgreSQL Ingestion")
    print("=" * 60)
